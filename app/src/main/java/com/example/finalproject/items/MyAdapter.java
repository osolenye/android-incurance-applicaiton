...
public class MyAdapter extends RecyclerView.Adapter<MyAdapter.ViewHolder> {
    ...
    public MyAdapter(List<Map<String, Object>> dataSet, OnItemClickListener listener) {
        this.dataSet = dataSet;
    ...
    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Map<String, Object> item = dataSet.get(position); // Определяем переменную item

        holder.textView1.setText(item.get("id").toString());
        holder.textView2.setText(item.get("industry").toString());
        holder.textView3.setText(item.get("start_date").toString());
        holder.textView4.setText(item.get("end_date").toString());

        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (listener != null) {
                    listener.onItemClick(item); // Используем item для передачи данных
                }
            }
        });
    }
